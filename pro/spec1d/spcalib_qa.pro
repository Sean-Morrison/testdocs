...
pro SpCalib_QA, run2d=run2d, fieldid=fieldid, mjd=mjd, rerun=rerun, nobkup=nobkup, epoch=epoch
    ...
    if keyword_set(fieldid) then begin
        ...
        fspecdir = get_field_spec_dir(getenv('BOSS_SPECTRO_REDUX'), run2d, fieldid, mjd, epoch=epoch)
        spallfile = djs_filepath(spallfile, root_dir = fspecdir)
        fdir = get_field_dir(getenv('BOSS_SPECTRO_REDUX'), run2d, fieldid)
        
        if keyword_set(epoch) then ctype = 'epoch' else ctype = 'daily'
        if keyword_set(epoch) then fsubdir = 'epoch'
        out_csv = djs_filepath(outname+'.csv', root_dir=getenv('BOSS_SPECTRO_REDUX'),$
                               subdir=[run2d,'summary',ctype])
        outname = outname+'-'+field_to_string(fieldid)+'-'+strtrim(mjd,2)
        outname = djs_filepath(outname+'.ps', root_dir=fdir, subdirectory=fsubdir)
        ...
        if keyword_set(epoch) then ctype = 'epoch' else ctype = 'daily'
        spallfile = djs_filepath(spallfile, root_dir=getenv('BOSS_SPECTRO_REDUX'),$
                                 subdir=[run2d,'summary',ctype])
        if not keyword_set(rerun) then begin
            out_csv = djs_filepath(outname+'.csv', root_dir=getenv('BOSS_SPECTRO_REDUX'),$
                                   subdir=[run2d,'summary',ctype])
            outname = djs_filepath(outname+'.ps', root_dir=getenv('BOSS_SPECTRO_REDUX'),$
                                   subdir=[run2d,'summary',ctype])
        endif else begin
    ...
    if ct_std ne 0 then begin
        ...
        if keyword_set(fieldid) then begin
            XYOUTS, 0.5,0.98, 'Field='+field_to_string(fieldid)+' MJD='+strtrim(mjd,2), $
                    CHARSIZE=1.2,  FONT=0, alignment=0.5,/NORMAL
        endif
