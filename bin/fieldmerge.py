...
def write_spAll(spAll, spline, spAll_lite, indir, run2d, datamodel, line_datamodel,
    ...
    if not exists:
        ...
        fits.HDUList([hdul,spAll]).writeto(spallfile+'.tmp', overwrite=True, checksum=True)
        del hdul, spAll
        gc.collect()
        rename(spallfile+'.tmp', spallfile)
...
=======
            ...
            fits.HDUList([hdul_lite, spAll_lite]).writeto(spalllitefile+'.tmp', overwrite=True, checksum=True)
            rename(spalllitefile+'.tmp', spalllitefile)
>>>>>>> Stashed changes
...
=======
            ...
            fits.HDUList([hdul_line,spline]).writeto(splinefile+'.tmp', overwrite=True, checksum=True)
            rename(splinefile+'.tmp', splinefile)
>>>>>>> Stashed changes
