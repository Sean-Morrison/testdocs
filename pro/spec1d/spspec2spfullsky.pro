...
CPU, TPOOL_NTHREADS = 1
  ...
  fdir = get_field_dir(topdir, '', coadd, /custom)
  if not keyword_set(mjd) then begin
    mjd = '*'
    spFieldname = filepath('spFullsky-'+strtrim(coadd,2)+'.fits',root_dir = fdir)
    plotsnfile = filepath('spSN2d-'+strtrim(coadd,2)+'.ps', root_dir = fdir)
    logfile = filepath('spSpec2spFullsky-'+strtrim(coadd,2)+'.log',root_dir = fdir)
  endif else begin
    spFieldname = filepath('spFullsky-'+strtrim(coadd,2)+'-'+strtrim(mjd,2)+'.fits',root_dir = fdir)
    plotsnfile = filepath('spSN2d-'+strtrim(coadd,2)+'-'+strtrim(mjd,2)+'.ps',root_dir = fdir)
    logfile = filepath('spSpec2spFullsky-'+strtrim(coadd,2)+'-'+strtrim(mjd,2)+'.log',root_dir = fdir)
  endelse
  ...
  splog, filepath('spSpec*.fits', root_dir=fdir,  subdirectory=['coadd', strtrim(mjd,2)])
  spSpecfiles = findfile(filepath('spSpec*.fits', root_dir=fdir, $
