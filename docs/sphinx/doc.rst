...
::
    ...
    Optional Quality Update (exclusive)
        At current only use if still exposing or don't run SOS after (skip and note in Night Log (and/or email) if uncertain):
    ...
    Optional Specialized Keys to Update 
         Only use if still exposing or don't run SOS after (skip and note in Night Log (and/or email) if uncertain):
