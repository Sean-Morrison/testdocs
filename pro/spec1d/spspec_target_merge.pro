...
pro spspec_target_merge, customplan, topdir=topdir
    ...
    if not keyword_set(custom_dir) then $
        custom_dir = get_field_dir(getenv('BOSS_SPECTRO_REDUX'),getenv('RUN2D'),yanny_par(hdr,'NAME'), /custom)
...
;            foreach exp, targ.FIELDS_LIST, i do begin
                ...
                foreach cid, targ.CATALOGID_LIST do begin
                    fdir = get_field_dir(topdir, '', fields[i])
                    spspecfile = filepath('spSpec-'+fmjds[i]+'-'+strtrim(cid,2)+'.fits', root_dir = fdir, $
        ...
        spSpecfiles = findfile(filepath('spSpec*.fits', root_dir=custom_dir, $
                                subdirectory=['coadd', strtrim(ec,2)]), $
                                count = nspSpec)
