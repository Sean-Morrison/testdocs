...
def read_zans(sp1d_dir, field, mjd):
    ...
    if ptt.exists(zansfile):
        ...
        try:
            ...
            zans.convert_bytestring_to_unicode()
...
def read_zline(sp1d_dir, field, mjd):
    ...
    if ptt.exists(zlinefile):
        ...
        try:
            ...
            zline.convert_bytestring_to_unicode()
...
def read_xcsao(sp1d_dir, field, mjd, spAll):
    ...
    if ptt.exists(XCSAOfile):
        ...
        try:
            ...
            XCSAO_tab.convert_bytestring_to_unicode()
...
def read_spline(splinefile, skip_line=False, clobber=False):
    ...
    if exists and not skip_line:
        try:
            ...
            spline.convert_bytestring_to_unicode()
            if len(spline) == 0:
                remove(splinefile)
                spline = None
        except:
...
def read_spall(spAllfile, clobber=False):
    ...
    if exists:
        try:
            ...
            spAll.convert_bytestring_to_unicode()
            if len(spAll) == 0:
                remove(spAllfile)
                spAll = None
        except:
...
def fieldmerge(run2d=getenv('RUN2D'), indir= getenv('BOSS_SPECTRO_REDUX'),
               ...
               custom=None, allsky=False, run1d=None, mjdstart=None):
    ...
    if epoch is True:
        fieldlist_file = ptt.join(indir, run2d, 'summary', 'epoch', 'fieldlist-'+run2d+'.fits')
    else:
        fieldlist_file = ptt.join(indir, run2d, 'summary', 'daily', 'fieldlist-'+run2d+'.fits')
    ...
    if logfile is None:
        if outroot is not None:
            ...
            if epoch is True:
                ...
                logfile = ptt.join(indir,  run2d, 'summary', custom, f'spAll_{custom}-{run2d}.log')
        else:
            if epoch is True:
    ...
    if allsky is False:
        ...
        if ptt.exists(fieldlist_file):
            ...
            try:
                ...
                flist.convert_bytestring_to_unicode()
            except:
                time.sleep(90)
                try:
    ...
    for i, row in enumerate(flist):
        if mjdstart is not None:
            if int(row['MJD']) < mjdstart:
                splog.log("Skipping Field:{row['FIELD']}  MJD:{row['MJD']} ({i+1}/{len(flist)})")
                continue
        ...
        cc = row['PROGRAM'] if custom is not None else None
        onefield = oneField(row, row['FIELD'], row['MJD'],
                            ...
                            merge_only=merge_only, custom = cc, allsky = allsky)
        if not merge_only:
            ff =row['FIELD'] if not allsky else custom
            write_spAll(onefield['spall'], onefield['spline'], None, indir, run2d, datamodel,
                        line_datamodel, outroot=None, field=ff, mjd = row['MJD'],
                        verbose=verbose, dev=dev, clobber=clobber, epoch=epoch, silent=True,
                        custom = cc, allsky = allsky, SDSSC2BV = SDSSC2BV)
        ...
        if lite is True:
                ...
                for i in range(mr):
                    ...
                    with warnings.catch_warnings():
                        ...
                        acols = {'ASSIGNED':'int','ON_TARGET':'int','VALID':'int',
                                 'DECOLLIDED':'int',
                                 'MOON_DIST':'float','MOON_PHASE':'float',
                                 'DELTA_RA':'float','DELTA_DEC':'float'}
                        for ac in acols:
                            if acols[ac] == 'int':
                                try:
                                    spAll_lite[i][ac] = str(min(np.array(spAll[i][ac].split()).astype(int)))
                                except:
                                    spAll_lite[i][ac] = '0'
                            elif acols[ac] == 'float':
                                try:
                                    spAll_lite[i][ac] = str(np.nanmean(np.array(spAll[i][ac].split()).astype(float)))
                                except:
                                    spAll_lite[i][ac] = 'nan'
                        try:
    ...
    if outroot is not None:
        ...
        if field is not None and mjd is not None:
            ...
            if epoch is True:
                ...
                spAll_dir  = ptt.join(indir, run2d, 'summary', custom)
                makedirs(spAll_dir, exist_ok = True)
            else:
                spAll_dir  = ptt.join(indir, run2d, 'summary', 'daily')
