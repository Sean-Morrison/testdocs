...
;
; PURPOSE:
;   Generate S/N plot for one plate from a FITS logfile written by SOSREDUCE.
;
; CALLING SEQUENCE:
;   sos_plotsn, logfile, plate, [ expnum=, plugdir=, plotfile= ]
;
; INPUTS:
;   logfile    - Logfile as written by SOSREDUCE.  This is a FITS file
 ...
 plotfile=plotfile, fps=fps, sdssv_sn2=sdssv_sn2, ccd=ccd, sn2_15=sn2_15
   ...
   if (Not keyword_set(fps)) then begin
      ...
      if strmatch(getenv('OBSERVATORY'), 'apo',/fold_case) eq 1 then begin
        ...
        plugmap = readplugmap(fullplugfile, 1, /deredden, /sostags, fibermask=fibermask, hdr=plhdr, ccd=ccd,savdir=savdir); included /deredden to match the SN2 in the html and plot-vivek
      endif else begin
        if not keyword_set(ccd) then ccd = 'b2'
        plugmap = readplugmap(fullplugfile, 2, /deredden, /sostags, fibermask=fibermask, hdr=plhdr, ccd=ccd,savdir=savdir); included /deredden to match the SN2 in the html and plot-vivek
      endelse
   endelse
   ;----------
